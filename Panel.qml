...
Panel {
  ...
  // VPN connection managed via `nmcli`. The profile keeps no stored secret
  // (vpn.secrets password-flags=1, "ask every time"), so bring-up opens a
  // floating terminal running a small script (Model.vpnConnectScript) that
  // prompts for the password itself and hands it to `nmcli ... --ask` over
  // stdin -- it never passes through this panel's process or disk.
  // `vpnConnecting` covers only tear-down, which runs quickly and silently
  // in the background.
  readonly property string vpnConnectionName: "DEIFVPN"
  property bool vpnConnected: false
  property bool vpnConnecting: false
  property string vpnError: ""

  // Ethernet device discovered via `nmcli device status`, kept separate from
  // `wiredDevice` (Quickshell.Networking) because the toggle itself is
  // driven through nmcli connect/disconnect rather than the Networking API.
  ...
  // Ethernet has no radio to kill, so "toggling" it means connecting or
  // disconnecting the wired device itself via nmcli.
  readonly property bool canToggleWired: networkManagerAvailable && wiredIface !== ""
  ...
  function refresh(scanWifi) {
    ...
    root.refreshVpnStatus()
    root.refreshWiredIface()
  ...
  function refreshVpnStatus() {
    if (!vpnStatusProc.running) vpnStatusProc.running = true
  }

  function updateVpnStatus(raw) {
    var lines = String(raw || "").split("\n")
    var found = false
    for (var i = 0; i < lines.length; i++) {
      var parts = lines[i].split(":")
      if (parts[0] === root.vpnConnectionName) { found = true; break }
    }
    root.vpnConnected = found
  }

  function toggleVpn() {
    if (root.vpnConnecting) return
    if (root.vpnConnected) {
      root.vpnError = ""
      root.vpnConnecting = true
      vpnActionProc.command = ["nmcli", "connection", "down", root.vpnConnectionName]
      vpnActionProc.running = true
      return
    }
    connectVpnInTerminal()
  }

  // No stored secret means bring-up always needs a password. Rather than let
  // `nmcli --ask` print its own prompt, this opens the same floating-terminal
  // helper the DNS "Custom" flow uses (see setDns) running a small script
  // (see Model.vpnConnectScript) that shows a shorter custom prompt, then a
  // reminder about the phone-side 2FA push, then feeds the typed password to
  // nmcli over stdin -- it never touches a QML property/process argv on this
  // machine, same guarantee `--ask` gave. The panel closes immediately after;
  // reopening it afterwards (or the VPN badge) reflects the outcome once
  // nmcli finishes in the terminal.
  function connectVpnInTerminal() {
    if (!root.bar) return
    var launcher = "omarchy-launch-floating-terminal-with-presentation"
    var script = Model.vpnConnectScript(Util.shellQuote(root.vpnConnectionName))
    root.bar.run(launcher + " " + Util.shellQuote(script))
    root.close()
  }

  // ----------------------------------------------------------- Ethernet

  function refreshWiredIface() {
    if (!wiredIfaceProc.running) wiredIfaceProc.running = true
  }

  function updateWiredIface(raw) {
    var lines = String(raw || "").split("\n")
    var connected = ""
    var available = ""
    var any = ""
    for (var i = 0; i < lines.length; i++) {
      ...
      var state = parts[2] || ""
      if (any === "") any = parts[0]
      if (state.indexOf("connected") === 0 && connected === "") connected = parts[0]
      else if (state !== "unavailable" && available === "") available = parts[0]
    }
    // Prefer the device actually carrying traffic, then one that's plugged
    // in but idle, then whatever ethernet device nmcli reports at all — a
    // machine can have more than one NIC (e.g. a USB-C dock plus onboard).
    root.wiredIface = connected || available || any
  }

  function toggleWired() {
    if (!root.wiredIface || root.wiredBusy) return
    root.wiredError = ""
    root.wiredBusy = true
    var action = root.wiredConnected ? "disconnect" : "connect"
    wiredActionProc.command = ["nmcli", "device", action, root.wiredIface]
    wiredActionProc.running = true
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateVpnStatus(text)
    }
  }

  // `nmcli connection up` blocks until the connection activates or fails,
  // which for DEIFVPN means sitting here until the phone-side 2FA prompt is
  // approved (or timed out on the NetworkManager side) — deliberately no
  // client-side timeout is imposed on top of that.
  Process {
    id: vpnActionProc
    ...
    onExited: function(exitCode) {
      ...
      root.vpnError = exitCode === 0 ? "" : ((vpnActionStderr.text || "").trim() || "VPN action failed")
      root.refreshVpnStatus()
    }
  }

  // Slower poll: VPN state rarely changes and this shells out to nmcli.
  // Runs whether or not the panel is open (unlike the other polls below) so
  // the bar's corner badge (vpnBadge) catches bring-up/tear-down finishing
  // in the detached floating terminal, which has no other way to notify
  // this panel when it's closed.
  Timer {
    id: vpnPoll
    interval: 4000
    repeat: true
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateWiredIface(text)
    }
  }

  Process {
    id: wiredActionProc
    stdout: StdioCollector { id: wiredActionStdout; waitForEnd: true }
    stderr: StdioCollector { id: wiredActionStderr; waitForEnd: true }
    onExited: function(exitCode) {
      root.wiredBusy = false
      root.wiredError = exitCode === 0 ? "" : ((wiredActionStderr.text || "").trim() || "Ethernet action failed")
      root.refreshWiredIface()
      root.refresh()
    }
  }
  ...
  Timer {
    ...
    repeat: true
    running: root.opened
    onTriggered: root.refreshWiredIface()
  }
  ...
  // Small VPN badge riding the bottom-right corner of the connection icon,
  // so Wi-Fi/Ethernet stays identifiable while VPN state is still visible
  // at a glance.
  Text {
    id: vpnBadge
  ...
  KeyboardPanel {
    ...
    Column {
      ...
      Item {
        ...
        // Same corner badge as the bar icon, scaled up to match the hero's
        // larger glyph.
        Text {
          id: heroVpnBadge
          visible: root.vpnConnected
          ...
          anchors.right: heroIcon.right
          anchors.bottom: heroIcon.bottom
        }
        ...
        RowLayout {
          ...
          Text {
            id: wifiToggleIcon
            visible: powerSwitch.visible
            textFormat: Text.PlainText
            text: "󰖩"
            color: root.bar.foreground
            font.family: root.bar.fontFamily
            font.pixelSize: Style.font.subtitle
            opacity: Networking.wifiEnabled ? 1 : 0.5
            Layout.alignment: Qt.AlignVCenter
          }
          ...
          // Ethernet has no radio to switch, so this connects/disconnects
          // the wired device directly via nmcli — same affordance as the
          // Wi-Fi power switch, just driven by a different mechanism.
          Text {
            id: wiredToggleIcon
            visible: wiredSwitch.visible
            textFormat: Text.PlainText
            text: "󰈀"
            color: root.bar.foreground
            font.family: root.bar.fontFamily
            font.pixelSize: Style.font.subtitle
            opacity: root.wiredConnected ? 1 : 0.5
            Layout.alignment: Qt.AlignVCenter
          }
          ...
          ToggleSwitch {
            ...
            PanelToolTip {
              ...
              text: root.wiredBusy
                ? (root.wiredConnected ? "Disconnecting Ethernet…" : "Connecting Ethernet…")
                : (root.wiredConnected ? "Turn Ethernet off" : "Turn Ethernet on")
              fontFamily: root.bar.fontFamily
            }
          }
        }
      ...
      // VPN connect/disconnect for DEIFVPN. Bring-up opens a floating
      // terminal running Model.vpnConnectScript, which prompts for the password
      // and any phone-side 2FA approval there; toggling off runs quietly in
      // the background since it needs no credentials.
      PanelSeparator {
        foreground: root.bar.foreground
      }
      ...
      Column {
        ...
        Row {
          ...
          Column {
            width: parent.width - vpnSwitch.width - parent.spacing
            anchors.verticalCenter: parent.verticalCenter
            spacing: Style.space(2)
            ...
            Text {
              ...
              width: parent.width
              text: {
                if (root.vpnConnecting) return "Disconnecting…"
                if (root.vpnError !== "") return root.vpnError
                return root.vpnConnected ? "Connected" : "Not connected"
              }
              color: root.vpnError !== "" ? root.bar.urgent : Qt.darker(root.bar.foreground, 1.4)
              font.family: root.bar.fontFamily
              font.pixelSize: Style.font.caption
              elide: Text.ElideRight
            }
