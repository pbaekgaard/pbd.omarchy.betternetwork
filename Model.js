...
// Runs inside the floating terminal opened for VPN bring-up (see
// connectVpnInTerminal). Replaces nmcli --ask's own secret prompt with a
// shorter one, adds a reminder about the phone-side 2FA push, then feeds the
// typed password to `nmcli --ask` over stdin -- same as typing it at nmcli's
// own prompt, just never touching argv or a QML property. `quotedName` must
// already be shell-quoted (see Util.shellQuote) by the caller.
function vpnConnectScript(quotedName) {
  return "IFS= read -rsp 'VPN password: ' pw; printf '\\n'" +
    "; printf 'Approve on Phone if the password was correct...\\n'" +
    "; printf '%s\\n' \"$pw\" | nmcli connection up " + quotedName + " --ask"
}
